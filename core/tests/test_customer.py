...
class CustomerTest(TestCase):
    ...
    def test_list(self):
        ...
        self.assertListEqual(
            bs.tbody.tr.text.split(),
            ["1", customer.name, "Обновить", "Удалить"]
        )

    def test_create(self):
        customer = factories.CustomerFactory.build()
        resp = self.client.post(
            reverse("core:customers_create"),
            {"name": customer.name}, follow=True
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_update(self):
        ...
        new_name = "Иван123"
        resp = self.client.post(
            reverse("core:customers_update", args=(customer.pk,)),
            {"name": new_name},
            follow=True,
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_delete(self):
        customer = factories.CustomerFactory()
        resp = self.client.post(
            reverse("core:customers_del", args=(customer.pk,)), follow=True
        )
        self.assertEqual(resp.status_code, 200)
        customer_db = models.Customer.objects.first()
