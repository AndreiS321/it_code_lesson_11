...
class SellerTest(TestCase):
    ...
    def test_list(self):
        ...
        self.assertListEqual(
            bs.tbody.tr.text.split(), ["1", seller.name, "Обновить", "Удалить"]
        )

    def test_create(self):
        seller = factories.SellerFactory.build()
        resp = self.client.post(
            reverse("core:sellers_create"), {"name": seller.name}, follow=True
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_update(self):
        ...
        new_name = "Иван123"
        resp = self.client.post(
            reverse("core:sellers_update", args=(seller.pk,)),
            {"name": new_name},
            follow=True,
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_delete(self):
        seller = factories.SellerFactory()
        resp = self.client.post(
            reverse("core:sellers_del", args=(seller.pk,)), follow=True
        )
        self.assertEqual(resp.status_code, 200)
        seller_db = models.Seller.objects.first()
