...
class ItemSearch(forms.Form):
    ...
    currency = forms.ChoiceField(label="Поиск по валюте", choices=(((None, "-"),) + models.Item.currency_choices),
                                 required=False)
    seller = forms.ModelChoiceField(
        queryset=models.Seller.objects.all(), label="Поиск по продавцу", required=False
    )
class OrderSearch(forms.Form):
    customer = forms.ModelChoiceField(
        queryset=models.Customer.objects.all(), label="Покупатель", required=False
    )
